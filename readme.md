# Resources
- elf header https://www.sco.com/developers/gabi/1998-04-29/ch4.eheader.html
- masm forum http://masm32.com/board/index.php?board=13.0
- overall good assembly material https://cs.lmu.edu/~ray/notes/x86assembly/
    - Best Masm Resource https://www.csie.ntu.edu.tw/~acpang/course/asm_2004/slides/
...
- voxel experiments httphttp://masm32.com/board/index.php?board=13.0s://www.youtube.com/watch?v=apXAoyKkjDs
...
## Playlists:
- Compilation and Linking https://www.youtube.com/watch?v=YoyKDZlXCUM&list=PLIz6U0slZNq2TS1zSUjZHgxBjAJL4nb92


# Questions:
- What is up with 32 bit application and using underscore when operating fromp assembly labels to C labels?
- and sometime when i'm using the windows kernell in 32bit 
...
# Commands
- ``gcc -c -g -E -S -save-temps``, -c to make obj file, -g to insert debug info, -E stop after cpp(C preprocessor)
...
- ``ld -emain test.o`` -e means entry point 
- ``readelf -a file.o | grep -e shared -e interpreter`` -a = all, everything that is needed
-``ldd file`` list dynamic dependencies
- ``nm --demangle file.p`` symbol table dump, U means undefined (needs/import), T is defined symbol


 Todo
- Try to read qword ptr that points to 0xFFFFFFFFFFFFFFFF into rax then do the same but with dword ptr and check if it's the same.
I'm expecting that when we read dword ptr we only get 0x000000fffffffffff, the bottom bits that is.

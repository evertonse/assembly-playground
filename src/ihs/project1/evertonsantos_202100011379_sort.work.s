...
.i_cmp: # if (i <= n-1) 
	cmp	esi, edx # first time means i <= n-1 ? which is not beause if fact  'i' it is 'n-1'
	jle	.return  # if i <= n-1 we jump to .return
	mov	r9d, esi # j = i
	xor	eax, eax # eax = 0
	sub	r9d, edx # j -= edx
.j_cmp:
	cmp	r9d, eax 
	jle	.L9      # if i <= eax, jump to L9
	mov	ecx, DWORD PTR [rdi+rax*4] # ecx = arr[j]
	mov	r8d, DWORD PTR 4[rdi+rax*4] # r8d = arr[j+1]
	cmp	ecx, r8d
	jle	.dont_swap # if arr[j] > arr[j+1] jump l3
# .swap
